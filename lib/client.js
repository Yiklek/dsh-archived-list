window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ArchivedSection(props) {
			...
			const [hostRows, setHostRows] = React.useState(null);
			...
			React.useEffect(() => {
				...
				let cancelled = false;
				rpcCall(props.ctx, "list", {}).then((res) => {
					if (cancelled) return;
					if (res !== null && typeof res === "object" && Array.isArray(res.rows)) setHostRows(res);
				}).catch((err) => {
			...
			const restoreRow = (row) => {
				...
				rpcCall(props.ctx, "restore", { sessionId: row.id }).then(() => {
					...
					let failed = false;
					if (sessionById[row.id] !== undefined) {
						const sessions = props.ctx.get("sessions");
						if (sessions !== undefined && sessions !== null && typeof sessions.open === "function") {
							try { sessions.open(row.id); } catch (err) { failed = true; setError(errText(err)); }
						}
					}
					if (!failed && typeof props.close === "function") props.close();
				}).catch((err) => {
					setBusy(null);
			...
			const runBatch = (kind) => {
				...
				const task = (async () => {
					...
					if (failed > 0) {
						...
						if (ids.length === 1 && sessionById[ids[0]] !== undefined) {
							const sessions = props.ctx.get("sessions");
							if (sessions !== undefined && sessions !== null && typeof sessions.open === "function") {
			...
			return React.createElement("div", { className: "archv-section" },
				React.createElement("h2", { className: "archv-heading" }, t("heading") + " · " + rows.length),
